...
死亡之后：选一名新冒险家试一下、再试一下。",
  ...
  worldgen_name="暴食" 
}`},_hoisted_1={class:"item-container"},_hoisted_2={class:"item-container"},_hoisted_3={class:"item-container"},_hoisted_4={class:"item-container"},_hoisted_5={class:"item-container"},_hoisted_6={class:"item-container"},_hoisted_7={class:"item-container"},_hoisted_8={class:"item-container"},_hoisted_9={class:"item-container"},_hoisted_10={class:"item-container"},_hoisted_11={class:"item-container"},_hoisted_12={class:"item-container"},_hoisted_13={class:"item-container"},_hoisted_14={class:"item-container"},_hoisted_15={class:"item-container"},_hoisted_16={class:"item-container"},_hoisted_17={class:"item-container"},_hoisted_18={class:"item-container"},_hoisted_19={class:"item-container"},_hoisted_20={class:"item-container"},_hoisted_21={class:"item-container"},_hoisted_22={class:"item-container"},_hoisted_23={class:"item-container"},_hoisted_24={class:"item-container"},_hoisted_25={class:"item-container"},_sfc_main={__name:"world",props:{formData:{type:Array,default:()=>[]},worldCount:{type:Number,default:0},maxWorlds:{type:Number,default:0},gameMode:{type:String,default:"endless"},theme:{type:String,default:"light"},tabWindowHeight:{type:Number,default:600}},setup(__props,{expose:__expose}){const props=__props,{mobile}=useDisplay(),{t}=useI18n();onMounted(async()=>{portFactor.value=props.worldCount+1,(props.formData?.length||0)!==0?(globalWorldIndex.value=props.formData.length,worldForm.value=props.formData,worldForm.value.forEach((k,v)=>{k.name=`World${v+1}`})):(globalWorldIndex.value=1,worldForm.value[0].name="World1",worldForm.value[0].isMaster=!0,worldForm.value[0].gameID=101,worldForm.value[0].worldName="Master",worldForm.value[0].serverPort=portFactor.value+GamePortFactor.serverPort,worldForm.value[0].masterServerPort=portFactor.value+GamePortFactor.masterServerPort,worldForm.value[0].authenticationPort=portFactor.value+GamePortFactor.authenticationPort,props.gameMode!=="custom"&&(worldForm.value[0].levelData=eval(props.gameMode).master,await handleWorldTabsEdit("","add"),worldForm.value[1].levelData=eval(props.gameMode).caves)),worldTabName.value=worldForm.value[0].name});const worldTabName=ref(""),worldForm=ref([{name:"",gameID:0,worldName:"",serverPort:0,masterServerPort:0,authenticationPort:0,isMaster:!1,encodeUserPath:!0,levelData:"",modData:""}]),worldFormRules=ref({gameID:[k=>k?!0:t("game.base.step2.gameID.required")],worldName:[k=>k?!0:t("game.base.step2.worldName.required")],serverPort:[k=>k?!0:t("game.base.step2.serverPort.required")],masterServerPort:[k=>k?!0:t("game.base.step2.masterServerPort.required")],authenticationPort:[k=>k?!0:t("game.base.step2.authenticationPort.required")]}),dynamicWorldRefs={},globalWorldIndex=ref(0),portFactor=ref(0),handleWorldTabsEdit=async(k,v)=>{if(v==="add"){globalWorldIndex.value=globalWorldIndex.value+1,portFactor.value=portFactor.value+1;const S=`World${globalWorldIndex.value}`;let n=!1;for(let C of worldForm.value)C.worldName==="Caves"&&(n=!0);dynamicWorldRefs[S]=ref(),worldForm.value.push({name:S,gameID:worldForm.value[worldForm.value.length-1].gameID+1,worldName:n?`Caves${worldForm.value[worldForm.value.length-1].gameID+1}`:"Caves",serverPort:GamePortFactor.serverPort+portFactor.value,masterServerPort:GamePortFactor.masterServerPort+portFactor.value,authenticationPort:GamePortFactor.authenticationPort+portFactor.value,isMaster:!1,encodeUserPath:!0,levelData:"",modData:""}),worldTabName.value=S}else if(v==="remove"){if(worldForm.value.length===1){showSnackbar("每个房间至少含有一个世界","error");return}const S=worldForm.value;worldForm.value=S.filter(n=>n.name!==k),delete dynamicWorldRefs[k]}worldLevelDataTabName.value="Code"},handleCreateWorld=k=>{for(let v of worldForm.value)v.name===worldTabName.value&&(k.clusterType==="endless"&&(k.worldType==="ground"&&(v.levelData=endless.master),k.worldType==="cave"&&(v.levelData=endless.caves)),k.clusterType==="survival"&&(k.worldType==="ground"&&(v.levelData=survival.master),k.worldType==="cave"&&(v.levelData=survival.caves)),k.clusterType==="relaxed"&&(k.worldType==="ground"&&(v.levelData=relaxed.master),k.worldType==="cave"&&(v.levelData=relaxed.caves)),k.clusterType==="wilderness"&&(k.worldType==="ground"&&(v.levelData=wilderness.master),k.worldType==="cave"&&(v.levelData=wilderness.caves)),k.clusterType==="lightsOut"&&(k.worldType==="ground"&&(v.levelData=lightsOut.master),k.worldType==="cave"&&(v.levelData=lightsOut.caves)),k.clusterType==="lavaarena"&&k.worldType==="ground"&&(v.levelData=lavaarena.master),k.clusterType==="quagmire"&&k.worldType==="ground"&&(v.levelData=quagmire.master))},worldLevelDataTabName=ref("Code"),visualizationType=ref(""),handleWorldTabChange=async k=>{if(k==="Visualization")try{for(let v of worldForm.value)if(v.name===worldTabName.value){visualizationType.value=getWorldType(v.levelData),await generateOverridesObj(v.levelData);break}}catch{showSnackbar("可视化配置加载失败","error")}k==="Code"&&await nextTick(()=>{for(let v of worldForm.value)if(v.name===worldTabName.value){v.levelData=beautifyLua(v.levelData);break}})},getWorldType=k=>{const v=/location\s*=\s*"([^"]*)"/,S=k.match(v);return S?S[1]:""},overridesObj=ref({}),generateOverridesObj=k=>{if(k==="")return;const v=luaparse.parse(k),S=extractOverrides(v);overridesObj.value=convertLuaTableToObject(S)};function extractOverrides(k){const S=k.body[0].arguments[0];for(const n of S.fields)if(n.key.type==="Identifier"&&n.key.name==="overrides")return n.value;return null}function convertLuaTableToObject(k){const v={};for(const S of k.fields){const n=S.key.name;v[n]=S.value.raw.replace(/"/g,"")}return v}const beautifyLua=k=>{let v=luamin.Beautify(k,{RenameVariables:!1,RenameGlobals:!1,SolveMath:!1});return v=v.replace(/--\[\[\n\t.+\n\t.+\n--\]\]\n\n\n\n\n/g,""),v=v.replace(/ = /g,"="),v=v.replace(/\t/g,"  "),v=v.replace(/overrides=\{ +(.+)\n/g,`overrides={
    $1
`),v=v.replace(/^(\w+)=(.+)\n/gm,`    $1=$2
`),v=v.replace(/(.+)\},\n/g,`$1
  },
`),v=v.replace(/^ +\n/gm,""),v},handleModelValueChange=debounce(k=>{const v=k.name,S=k.value;for(let n of worldForm.value)if(n.name===worldTabName.value){const C=luaparse.parse(n.levelData),B=extractOverrides(C);for(let Y of B.fields)Y.key.name===v&&(Y.value.raw=`"${S}"`);n.levelData=astToLua(C);break}},100),astToLua=(k,v=0)=>{const S="    ".repeat(v);switch(k.type){case"Chunk":return k.body.map(n=>astToLua(n,v)).join(`
