...
function install_dmp() {
	...
	github_url=$(echo "$release_json" | jq -r '.assets[] | select(.name == "dmp_darwin.tgz") | .browser_download_url')
	if [[ -z "$github_url" ]]; then
		echo_red "未找到下载文件，请检查网络连接"
		exit 1
	fi
	github_digest=$(echo "$release_json" | jq -r '.assets[] | select(.name == "dmp_darwin.tgz") | .digest' | awk -F':' '{print $2}')
	...
	if download "${url}" "dmp_darwin.tgz" 10; then
		# 验证 SHA-256
		file_digest=$(sha256sum dmp_darwin.tgz | awk '{print $1}')
	...
	set -e
	tar zxvf dmp_darwin.tgz >/dev/null
	rm -f dmp_darwin.tgz
	chmod +x "$ExeFile"
	set +e
}
